...
class ModularParameterHandler<T>(
    val parsers: Map<KParameter, Builder<*>>,
    ...
    override fun parse(parameters: Parameters, headers: Headers): T {
        return constructor.callBy(parsers.mapValues {
            val value = it.value.build(
                ...
                it.key.remapOpenAPINames(parameters.toMap() + headers.toMap().entries.groupBy {
                    it.key.lowercase(
                        Locale.getDefault()
                    )
    ...
    override fun getParameters(
        ...
        val schemaBuilder =
            provider.ofType<FinalSchemaBuilderProviderModule>().last().provide(apiGen, provider)
        val kType = constructor.returnType

        fun createParam(
            param: KParameter,
            `in`: ParameterLocation,
            config: (ParameterModel<*>) -> Unit
        ): ParameterModel<*> {
    ...
    companion object {
        private val paramAnnotationClasses =
            hashSetOf(HeaderParam::class, PathParam::class, QueryParam::class)
    }
}
