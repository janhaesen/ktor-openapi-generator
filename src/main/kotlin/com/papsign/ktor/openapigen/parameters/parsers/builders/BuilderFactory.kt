...
interface BuilderFactory<out T : Builder<S>, S> where S : ParameterStyle<S>, S : Enum<S> {
    ...
    fun buildBuilderForced(type: KType, explode: Boolean): T = buildBuilder(type, explode)
        ?: error(
            "No ${this.javaClass.declaringClass?.simpleName ?: this.javaClass.simpleName} Builder exists for type $type"
        )
}
