...
interface Responder {
    suspend fun <TResponse : Any> respond(
        response: TResponse,
...
suspend inline fun <reified TResponse : Any> OpenAPIPipelineResponseContext<TResponse>.respond(
    ...
    val statusCode = route.provider.ofType<StatusProvider>().lastOrNull()
        ?.getStatusForType(getKType<TResponse>())
        ?: TResponse::class.findAnnotation<Response>()?.statusCode?.let {
